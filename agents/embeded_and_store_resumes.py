...
RESUME_FOLDER = "CVs1"  # new folder with remaining resumes
FIELD_MODEL = "mistral"  # model for field extraction
EMBED_MODEL = "distilbert-base-uncased"  # embedding model
...
""".strip()
    ...
    try:
        response = chat(model=FIELD_MODEL, messages=[{"role": "user", "content": prompt}])
...
def get_embedding(text):
    try:
        ...
        print(f"❌ Error generating embedding with DistilBERT: {e}")
        return None
